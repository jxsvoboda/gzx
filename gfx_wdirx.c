...
static int dd_init(HWND wnd) {
  ...
  ddrval=IDirectDraw_SetDisplayMode(lpdd,ddxr,ddyr,8);
  if(ddrval!=DD_OK) {
    IDirectDraw_Release(lpdd);
    return -1;
  }
...
static int mwin_init(void) {

  hinst=GetModuleHandle(NULL);
  hmem=LocalAlloc(LPTR,sizeof(WNDCLASSEX));
  if(!hmem) return -1;

  winclass=(LPWNDCLASSEX)LocalLock(hmem);
  ...
  winclass->lpszMenuName=(LPSTR)"gfxwinmenu";
  ...
  if(!(hwnd=CreateWindowEx(WS_EX_TOPMOST,WINDOW_CLASS_NAME,"GZXwin",
                 WS_VISIBLE | WS_POPUP,
                 0,0,
